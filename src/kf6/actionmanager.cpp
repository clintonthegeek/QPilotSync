...
void ActionManager::setupFileActions()
{
    ...
    // Recent Profiles (menu will be populated dynamically)
    // This is handled via KRecentFilesAction or manually
...
{
    // Tabbed View
    QAction *tabbedView = new QAction(i18n("&Tabbed View"), this);
    tabbedView->setCheckable(true);
    m_actionCollection->addAction(QStringLiteral("view_tabbed"), tabbedView);

    // Show Log
    QAction *showLog = new QAction(i18n("Show &Log"), this);
    m_actionCollection->addAction(QStringLiteral("view_show_log"), showLog);
    ...
    // Toggle Sidebar
    QAction *toggleSidebar = new QAction(i18n("Show &Sidebar"), this);
    toggleSidebar->setCheckable(true);
    toggleSidebar->setChecked(true);
    m_actionCollection->addAction(QStringLiteral("view_toggle_sidebar"), toggleSidebar);
    m_actionCollection->setDefaultShortcut(toggleSidebar, QKeySequence(Qt::Key_F9));
