...
| ✅ **E.19** | **Landed 2026-05-21.** Three-pass docs supersession. Pass 1: legacy conduit / SDK docs (`CONDUIT_PLUGIN_DESIGN.md`, `plugin-developer-guide.md`, `sdk-plugin-guide.md`, `sdk-shadowstan-advisory.md`) + three pre-Phase-E TODO files moved to `docs/archived/`. Pass 2: campaign-coordination artifacts curated into `docs/archived/engine-merger-2026/` with a provenance README (20 files: FINDINGS.md full + WP-focused phase plans + merge plan + WP-specific FINDINGS index). Pass 3: refreshed architecture docs for post-merger reality — new `docs/PLUGIN_ABI.md` describes `WildPalms::IPlugin`/`IPluginAction` and the upstream `Kalburator::Plugin` contract; rewrote `docs/ARCHITECTURE_2026.md` (v3.0) and `docs/SYNC_ENGINE_ARCHITECTURE.md` for `PalmRuntime` + libkalburator; refreshed `docs/LIBKALBURATOR.md` to drop the stale 2026-04-21 snapshot and reflect FetchContent consumption. Integration plan's Phase E row flipped to ✅. | WP | E.16 (was E.18, now voided) | New docs committed; integration plan's Phase E row ✅. |
